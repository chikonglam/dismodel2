...
public class MogiSourceDialog2 extends MogiSourceDialogBase implements RestorableSourceDialog, GeoPosClickListener, DataChangeEventFrier{
	...
	public MogiSourceDialog2(Window owner, String title, int modelIndex, AllGUIVars allGuiVars){
		...
		SourceDialogUtils.parseValsIntoUIBoxes(curMogi.getEast(), curLB.getEast(), curUB.getEast(), CBFixX, txtValX, txtLBX, txtUBX);
		SourceDialogUtils.parseValsIntoUIBoxes(curMogi.getNorth(), curLB.getNorth(), curUB.getNorth(), CBFixY, txtValY, txtLBY, txtUBY);
		SourceDialogUtils.parseValsIntoUIBoxes(-curMogi.getUp(), -curUB.getUp(), -curLB.getUp(), CBFixC, txtValC, txtLBC, txtUBC);
	...
	public MogiPoint getSource(){
		...
		volChange = parseJTextField(txtVolChg);			//Change this line for UB and LB
		if (Double.isNaN(volChange)) volChange = 0d;
		
		MogiPoint sourceRtn = new MogiPoint(east.number, north.number, -depth.number, volChange);	//Change this line for UB and LB
		return sourceRtn;
	}
	...
	public MogiPoint getUpperBound(){
		...
		MogiPoint sourceRtn = new MogiPoint(east.ub, north.ub, -depth.lb, Double.MAX_VALUE);	//Change this line for UB and LB
		return sourceRtn;
	}
	...
	public MogiPoint getLowerBound(){
		...
		MogiPoint sourceRtn = new MogiPoint(east.lb, north.lb, -depth.ub, -Double.MAX_VALUE);	//Change this line for UB and LB
		return sourceRtn;
	}
	
	//TODO: really make bounds work
	public void showSource(DisplacementSolver sourceIn, int indexInSimModel, DisplacementSolver sourceLB, DisplacementSolver sourceUB) {
        // TODO improve this
...
//	@Override
//	public void mouseClick(java.awt.geom.Point2D.Double coords, boolean isLatLng) {
//        String x, y;
//        if (isLatLng) {
...
//        }
//        
//        if (filledXField != null && filledXField.isEnabled()) filledXField.setText(x);
//        if (filledYField != null && filledYField.isEnabled()) filledYField.setText(y);
//	}
    ...
    protected void btnOKActionPerformed(java.awt.event.ActionEvent evt) {                                      
    	if (boundsAreValid()){
            ...
            .showMessageDialog(this ,
                    "Invalid bounds and guesses.  Please doublecheck.",
                    "Invalid bounds",
	...
	protected void adjustTxtBoxesWithChkbox(javax.swing.JCheckBox fixedChkBox, javax.swing.JTextField valTxt, javax.swing.JTextField lbTxt, javax.swing.JTextField ubTxt){
	...
	//----------------------
	private NumberWithRange parseChkNTxtBoxes(javax.swing.JCheckBox fixedChkBox, javax.swing.JTextField valTxt, javax.swing.JTextField lbTxt, javax.swing.JTextField ubTxt){
		final boolean fixed = fixedChkBox.isSelected();
		if (fixed){
	...
	public void latLonClicked(LLH location) {
		if (unitsAreXY) return;
		String x = String.format("%4.5f", location.getLongitude() );
		String y = String.format("%4.5f", location.getLatitude() );
	...
	public void LocalENUClicked(LocalENU location) {
		...
		String x = String.format("%4.5f", location.getEasting());
		String y = String.format("%4.5f", location.getNorthing());
