...
/**
 * Compute the deformation due to a pressurized finite spheroid forward model
 * based on eq. (52) and (53) by McTigue (1988)
 * 
 * @author clam-PR
 * 
 */
...
public class McTigueSphere extends DisplacementSolver {
    ...
    final private static ArrayList<Integer> LINVAR = new ArrayList<Integer>(Arrays.asList((new Integer[] { new Integer(
	    VOLUME_IDX) })));
     ...
     * 
     * @param refParams
     *            Reference to the array, the sphere will change whenever this
     *            changes
     * @param shearModulus
     */
    ...
    /**
     * Construct a McTigue Sphere by specifying all the parameters, and set
     * shearModulus to 1
     * 
     * @param center
     * @param radius
	...
	return getName() + " (McTigueSphere)[E=" + msp[EAST_LOC_IDX] + ", N=" + msp[NORTH_LOC_IDX] + ", U="
	        + msp[UP_LOC_IDX] + ", DV=" + msp[VOLUME_IDX] + ", R=" + radius + ", shearModulus=" + shearModulus
	        + "]";
    }

    @Override
    public AbstractDrawable toAbstractDrawable(DisplayStateStore displaySettings) {
	return new CrossHair(new Coord3d(msp[EAST_LOC_IDX], msp[NORTH_LOC_IDX], msp[UP_LOC_IDX]), (float) radius,
	        getName(), displaySettings);
    }
	...
	final LatLon centerLatLon = LatLon.fromDegrees(centerLLH.getLatitude().toDeg(), centerLLH.getLongitude()
	        .toDeg());
