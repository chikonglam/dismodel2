...
import gov.usgs.dismodel.geom.overlays.jzy.ColorUtil;
...
public class OkadaFault3 extends DisplacementSolver implements Fault {
    ...
    final public static int STRIKE_IDX = 7; // angle (direction) of strike
                                            // movement
    final public static int DIP_IDX = 8; // angle in degrees from horizontal, of
                                         // slip
    final public static int ASPECT_RATIO_IDX = 9;
    final public static int LENGTH_IDX = 10;
    final public static int WIDTH_IDX = 11; // typically down (or up depending
                                            // on convention used)
    final public static int STRIKE_SLIP_IDX = 12; // horizontal distance slipped
    final public static int DIP_SLIP_IDX = 13; // distance slipped, typically
                                               // somewhat vertically
    final public static int OPENING_IDX = 14;

    // class constant
    // --------------
    final private static ArrayList<Integer> LINVAR = new ArrayList<Integer>(Arrays.asList(new Integer[] {
    ...
    public OkadaFault3(double x1, double y1, double x2, double y2, double xC, double yC, double depth,
            boolean isTopCoords, double strike, double dip, double aspectRatio, double length, double width,
            double strikeSlip, double dipSlip, double opening) {
        this();
    ...
    public AbstractDrawable toAbstractDrawable(DisplayStateStore displaySettings) {
        ...
        return new FaultViewable(bottomCenter, length, width, strike, dip, ColorUtil.toJzyColor(displaySettings
                .getSourceColor()), this.getName());
    }
    ...
    public String toString() {
        ...
        if (!Double.isNaN(msp[STRIKE_IDX]))
            outStr += String.format("Strike(deg)=%.3e, ", msp[STRIKE_IDX]);
        if (!Double.isNaN(msp[DIP_IDX]))
            outStr += String.format("Dip(Deg)=%.3e, ", msp[DIP_IDX]);
        if (!Double.isNaN(msp[ASPECT_RATIO_IDX]))
            outStr += String.format("AR=%.3e, ", msp[ASPECT_RATIO_IDX]);
        if (!Double.isNaN(msp[LENGTH_IDX]))
            outStr += String.format("Length=%.3e, ", msp[LENGTH_IDX]);
        if (!Double.isNaN(msp[WIDTH_IDX]))
            outStr += String.format("Width=%.3e, ", msp[WIDTH_IDX]);
        outStr += String.format("SS=%.3e, DS=%.3e, TS=%.3e, ShearModulus=%.3e]", msp[STRIKE_SLIP_IDX],
                msp[DIP_SLIP_IDX], msp[OPENING_IDX], shearModulus);
        return outStr;
    }
    ...
    // okada 85 stuff
    private displacement okada85(final double x0, final double y0, final double z0, final double L, final double W,
            final double phiDeg, final double deltaDeg, final double Us, final double Ud, final double Ut,
            final double mu, final double nu, final double x, final double y) {
        ...
        // % rotate the coordinate system to be coherent with the model
        // coordinate
    ...
    private displacement ok85feeder(final double csi, final double eta, final double q, final double mu,
            final double lambda, final double Us, final double Ud, final double Ut, final double sinDelta,
            final double cosDelta, final double tanDelta) {
        // % *** GENERAL PARAMETERS
        // **************************************************
        // % Okada (1985), equation (30)
        ...
        // double Ac = (((2d * R) + csi) / ((Math.pow(R, 3d)) * (Math.pow((R +
        // csi), 2d)))); //not used for just displacements
        // double An = (((2d * R) + eta) / ((Math.pow(R, 3d)) * (Math.pow((R +
        // eta), 2d)))); //not used for just displacements

        /*
         * //not used for just displacements //% Okada (1985), equation (40) and
         * (41) //% check singularity for cos(delta)=0 double K1, K2, K3; if
         * (cosDelta < 1E-16){ //% Okada (1985), equation (41) K1 = (((alpha *
         * csi) * q) / (R * (Math.pow((R + dtilde), 2d)))); K3 = ((alpha *
         * ((sinDelta) / (R + dtilde))) * (((Math.pow(csi, 2d)) / (R * (R +
         * dtilde))) - 1d)); } else { K3 = ((alpha * (1d / (cosDelta))) * (((q *
         * Reta) / R) - (ytilde / (R * (R + dtilde))))); K1 = ((alpha * (csi /
         * (cosDelta))) * ((1d / (R * (R + dtilde))) - (((sinDelta) * Reta) /
         * R))); } K2 = ((alpha * (((-((sinDelta))) / R) + (((q * (cosDelta)) *
         * Reta) / R))) - K3);
         */
        ...
        if (cosDelta < 1E-16) { // % Okada (1985), equation (29)
            ...
            if (Math.abs(csi) < 1E-16) { // % check singularity condition (ii),
                                         // pp 1148, Okada (1985)
                I5 = 0;
            } else {
                I5 = ((alpha * (2d / (cosDelta))) * (Math
                        .atan((((eta * (X + (q * (cosDelta)))) + ((X * (R + X)) * (sinDelta))) / ((csi * (R + X)) * (cosDelta))))));
            }
        ...
        if (Ut != 0d) {
            displacement curDisp = ok8527(csi, sinDelta, cosDelta, dtilde, eta, q, R, Rcsi, Reta, ytilde, Ut, I1, I3,
                    I5);
            accumDisp.add(curDisp);
        }
    ...
    private OkadaFundamentalVars toFundamentalVars() {
        ...
        if (!Double.isNaN(x1In)) { // 1 or 2 ends are given
            ...
            x0 = x1In;
            y0 = y1In;
            if (!Double.isNaN(x2In)) { // 2 ends are given, calculate L and
                                       // strike
            // twopointsGiven = true;
                double dX = (x2In - x1In);
                double dY = (y2In - y1In);
            ...
            // twopointsGiven = false;
                if (Double.isNaN(lengthIn)) { // calculate length if it's not
                                              // defined
                // lengthGiven = false;
    ...
    private class OkadaFundamentalVars {
        ...
        public OkadaFundamentalVars(double x0, double y0, double z0, double l, double w, double phi, double delta,
    ...
    private displacement ok8525(double csi, final double sinDelta, final double cosDelta, double dtilde, double eta,
            double q, double R, double Reta, double ytilde, double U1, double I1, double I2, double I4) {
        double ux;
        if (Math.abs(q) < 1E-16) {
    ...
    private displacement ok8526(double csi, final double sinDelta, final double cosDelta, double dtilde, double eta,
            double q, double R, double Rcsi, double ytilde, double U2, double I1, double I3, double I5) {
        double ux, uy, uz;
        ux = ((-((U2 / (2d * Math.PI)))) * ((q / R) - ((I3 * (sinDelta)) * (cosDelta))));

        if (Math.abs(q) < 1E-16) { // % check singularity condition (i), pp
                                   // 1148, Okada (1985)
            uy = ((-((U2 / (2d * Math.PI)))) * ((((ytilde * q) * Rcsi) / R) + (((-(I1)) * (sinDelta)) * (cosDelta))));
            uz = ((-((U2 / (2d * Math.PI)))) * ((((dtilde * q) * Rcsi) / R) + (((-(I5)) * (sinDelta)) * (cosDelta))));
        } else {
            uy = ((-((U2 / (2d * Math.PI)))) * (((((ytilde * q) * Rcsi) / R) + ((cosDelta) * (Math
                    .atan(((csi * eta) / (q * R)))))) - ((I1 * (sinDelta)) * (cosDelta))));
            uz = ((-((U2 / (2d * Math.PI)))) * (((((dtilde * q) * Rcsi) / R) + ((sinDelta) * (Math
                    .atan(((csi * eta) / (q * R)))))) - ((I5 * (sinDelta)) * (cosDelta))));
        }
        return new displacement(ux, uy, uz);
    }

    private displacement ok8527(double csi, final double sinDelta, final double cosDelta, double dtilde, double eta,
            double q, double R, double Rcsi, double Reta, double ytilde, double U3, double I1, double I3, double I5) {
        double ux = ((U3 / (2d * Math.PI)) * ((((Math.pow(q, 2d)) * Reta) / R) - (I3 * (Math.pow((sinDelta), 2d)))));
        double uy, uz;
        if (Math.abs(q) < 1E-16) { // % check singularity condition (i), pp
                                   // 1148, Okada (1985)
            uy = ((U3 / (2d * Math.PI)) * ((((((-(dtilde)) * q) * Rcsi) / R) - ((sinDelta) * (((csi * q) * Reta) / R))) - (I1 * (Math
                    .pow((sinDelta), 2d)))));
            uz = ((U3 / (2d * Math.PI)) * (((((ytilde * q) * Rcsi) / R) + ((cosDelta) * (((csi * q) * Reta) / R))) - (I5 * (Math
                    .pow((sinDelta), 2d)))));
        } else {
            uy = ((U3 / (2d * Math.PI)) * ((((((-(dtilde)) * q) * Rcsi) / R) - ((sinDelta) * ((((csi * q) * Reta) / R) - (Math
                    .atan(((csi * eta) / (q * R))))))) - (I1 * (Math.pow((sinDelta), 2d)))));
            uz = ((U3 / (2d * Math.PI)) * (((((ytilde * q) * Rcsi) / R) + ((cosDelta) * ((((csi * q) * Reta) / R) - (Math
                    .atan(((csi * eta) / (q * R))))))) - (I5 * (Math.pow((sinDelta), 2d)))));
        }
        return new displacement(ux, uy, uz);
    }
