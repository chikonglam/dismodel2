...
/** Calculates slips of modeled fault-blocks given GPS data, a Green's function, and 
 * some or all of various constraints.
 * 
 * The GPS readings make our equations non-homogeneous,
 * requiring us to use a quadratic program instead of the classic linear program. 
 * This translates our non-homogeneous linear system of equations into the standard
 * Quadratic Program, which various libraries can solve, in order to apply several 
 * different kinds of bounds and equality constraints to the solution.
 * See A REFLECTIVE NEWTON METHOD FOR MINIMIZING A QUADRATIC FUNCTION SUBJECT 
 * TO BOUNDS ON SOME OF THE VARIABLES by THOMAS F. COLEMAN AND YUYING Li
 * SIAM J. OPTIMIZATION, Vol. 6, No. 4, pp. 1040-1058, November 1996
 * which notes the use of AtA = H = Q, and c = -ATb to translate the minimization of
 * the sum of squares of the residual, Ax - b, to minimizing [X]T[H][X] - [C]T[X]
 * 
 * @author Chris Forden */
public class EqualityAndBoundsSlipSolver extends 
            ConstrainedLinearLeastSquaresSolver {
	...
	/** Matrices for constraints	 */
		...
		/** Created and cached on demand when a getBound...() 
        ...
        /** Created and cached on demand when a getEqualityConstraint...() 
         * method is first called */
	 ...
	 * 		
	 * We require the constructor to take the main parameters because setting them more
	 * than once could make the code prone to various subtle bugs.
	 * 
	 * @param greensMatrix
	 ...
	 *            
	 */
	public EqualityAndBoundsSlipSolver(double[][] greensMatrix, 
	...
	public void setGreensFunction(double[][] greensMatrix) {
        ...
        H = (JamaMatrix) At.multiplyRight(
                (MatrixStore<Double>)(greensFunct));
    }
    ...
    public void setMeasuredDisplacements(double[] disps) {
        ...
        c = (JamaMatrix) At.multiplyRight(
                (MatrixStore<Double>)(this.measuredDisplacements));
    }
	...
	/* TODO: Check for conflicts: equality vs. inequality, 
	 * block bounds whose sum violates moment bounds.
	 */
	...
	public String getSolutionNotes() {
		...
		String iterSse = "After " + Integer.toString(solutionResult.
		        getIterationsCount()) + " iterations, the raw SSE= " + 
		        Double.toString(calcSSE(solutionResult.getSolution()));
		...
		if (AI != null) {
		    ...
		    for (int i = 0; i < getSolutionSize(); i++) {
		        ...
		        if (AI.doubleValue(i, i) < LOWER_BOUND_THRESHOLD && 
		                -BI.doubleValue(i, 0) + tol > ithPred) {
                    solNotes.append("   ");
                    solNotes.append(i + 1);
                    solNotes.append(": ");
		    ...
		    solNotes.append("  The preceding " + numActivelyBounded +
		            " solution values, were within " + 
		            tol + " of their constraints.  Therefore they probably " +
		            "were actively limited by those bounds.");
		}
     ...
     * 
	 * @param faultBlock Index into the grid of cells ("blocks") in the fault 
	 * 		being modeled. Zero-based.
	 * @param lb The lower bound for the fault-block being constrained.
	 * @throws SolverException 
	 * @see  #setUpperBoundForABlockSlip()
     * @see  #setMomentLowerBound()
     * @see  #setMomentUpperBound()
	 */
	public void setLowerBoundForABlockSlip(int faultBlock, double lb) 
			throws SolverException {
	    double ub = getUpperBound(faultBlock);
	    if ( ! Double.isNaN(ub) && ub < lb && checkConstraints)
            throw new SolverException("Lower bound greater than " +
            		"upper bound for a subfault's slip");
	    addBoundForABlockSlip(faultBlock, lb, MAKE_LOWER_BOUND);
	}
	
	/** This makes no attempt yet to notice whether bounds being set overwrite
	 * or conflict with previously set bounds or inequalities.
     * 
	 * @param index Which fault-block is being constrained. Zero-based. 
	 * @param ub The new upper bound for the fault-block being constrained.
	 * @throws SolverException 
	 * @see  #setLowerBoundForABlockSlip()
     * @see  #setMomentLowerBound()
     * @see  #setMomentUpperBound()
	 */
	public void setUpperBoundForABlockSlip(int index, double ub) 
			throws SolverException {
		double lb = getLowerBound(index);
		if ( ! Double.isNaN(lb) && ub < lb && checkConstraints)
            throw new SolverException("Upper bound less than " +
                    "lower bound for a subfault's slip");
     ...
     */
	private void addBoundForABlockSlip(int index, double bound, 
			int makeWhichBound) throws SolverException {
		...
		case MAKE_UPPER_BOUND:
			if (getSolutionSize() < 1)
				throw new SolverException("EqualityAndBoundsSlipSolver." +
						"addBoundForABlockSlip bad solutionSize");
		...
		default:
			throw new SolverException("EqualityAndBoundsSlipSolver." +
					"addBoundForABlockSlip bad bound.");
		}
	}
    ...
    */ 
	...
	private void assertConstrainability() {
	    if (getSolutionSize() < 1)
	        throw new SolverException("You must either set the Green's " +
	        		"function or call setNumberOfModeledParameters() " +
	        		"before setting constraints");
	    if (subfaultToLowerBIndices != null || subfaultToUpperBIndices != null ||
	            momentLbIndex != -1 || momentUbIndex != -1 || 
	            subfaultToEqualityIndices != null || momentEqualityIndex != -1)
     ...
     *            A matrix the same size as the Green's function to be set later.
     */
	 ...
	 * The 2nd square of rows (below the first) are for upper bounds.  Below those
	 * squares, the 2nd-to-bottom row is for the lower bound of the moment.  The
	 * bottom row is for the upper bound of the moment.
	 */
	...
	private void maybeCreateInequalityMatrices() {
        assertConstrainability();
	    if (subfaultToLowerBIndices != null || subfaultToUpperBIndices != null) 
	        throw new SolverException("Don't add bounds after solving without " +
	...
	/**
	 * Put max or min constraint on the total energy of the fault motion 
	 * (summed over all the sub-fault blocks).
	 *  
	 * @param mb
	 ...
	 * @throws SolverException
	 */
	private void setMomentBound(double mb, double makeWhichBound) 
				throws SolverException {
		maybeCreateInequalityMatrices();
		...
		if (BI.getRowDim() != AI.getRowDim())
			throw new SolverException("EqualityAndBoundsSlipSolver." +
					"setMomentLowerBound: BI.getRowDim() " +
					"!= AI.getRowDim()");
	 ...
	 * their combined area, times the material's shear modulus.  
	 * Use this method to constrain that sum.  
     * 
     * If you constrain the moment, you must call setSubfaultAreas() before 
     * solving.
     * 
	 * @param mub Moment upper bound
	 * @param totArea The sum of the areas for all the fault-blocks being modeled.
	 * @param shearModulus The average shear modulus for the rock or material of the fault.
	 * @throws SolverException 
	 * @see  #setMomentLowerBound()
     * @see  #setLowerBoundForABlockSlip()
     * @see  #setUpperBoundForABlockSlip()
	...
	/** Constrain a fault-block to have exactly the specified slip.
	 * This is a kind of equality constraint.
	 * This makes no attempt to notice whether the bounds being set, overwrite
	 * or conflict with, previously set constraints.
     * 
	 * @param faultBlock An index into the grid of cells modeling the fault
	 * @param slip How far the block moves
	 * @throws SolverException
	 */
	...
	/** Constrain the sum of slips of all fault-blocks being modeled, 
	 * multiplied by their areas and the (shear) modulus of the rock, to equal 
	 * the specified moment-value.
	 * This is a kind of equality constraint.
	 * This makes no attempt to notice whether the bounds being set, overwrite
	 * or conflict with, previously set constraints.
     * 
	 ...
	 * @throws SolverException 
	 */
	public void setMoment(double moment, double shearModulus) 
	            throws SolverException {
		maybeCreateEqualityMatrices();
		setCheckShearModulus(shearModulus);
	...
	private void maybeCreateEqualityMatrices() {
        ...
        if (subfaultToEqualityIndices != null)
            throw new SolverException("Constraint applied after " +
                "solving without removing old constraints");        
	...
	/** Set and check coefficients related to moment constraints.
	 * Prevent client from passing one set of coefficient values to 
	 * setMomentLowerBound() but passing a non-equal set to setMomentUpperBound().
	 * Also prevent passing zero values. */ 
	private void setCheckShearModulus(double shearModulus) {
		...
		if (shearModulus == 0.0)
			throw new SolverException("Zero modulus passed as a " +
					"moment constraint.");

		if (modulus == 0.0)
			modulus = shearModulus;
		else
			if (modulus != shearModulus)
				throw new SolverException("New non-zero modulus set before " +
    ...
    public double[] solve() {
        ...
        c = (JamaMatrix) At
                .multiplyRight((MatrixStore<Double>) (measuredDisplacements));

        builder = new QuadraticSolver.Builder(H, c);
        if (AI != null) {
            if (BI == null)
                throw new SolverException("One but not both inequality "
                        + "matrices had been set.");
            else {
        ...
        if (AE != null) {
            if (BE == null)
                throw new SolverException("One but not both equality "
                        + "matrices had been set.");
            else {
                purgeUnusedEqualityRows();
    ...
    private void checkAreasSet(String constraintKind) {
        if (subfaultAreas == null) {
            throw new SolverException("Since you set an " + constraintKind +
                		" constraint on the moment, you should " +
                		"setSubfaultAreas before Solving.");
        }
        if (subfaultAreas.length != this.greensFunct.getColDim())
            throw new SolverException(subfaultAreas.length + " subfault areas " +
                    "had been set, but there are " + greensFunct.getColDim() +
                    " subfaults.");
    }

    
    /** Culls the rows of the inequality matrix that do not correspond
     * to any bound.  We need to eliminate unused rows so ojAlgo's quad. prog.
     * solver doesn't take 40 times longer to run.  Also see 
     * purgeUnusedEqualityRows()  */
        ...
        for (int i = 0; i < rows; i++) {
            ...
            if (numLowersThisRow > 0) {
                if (numUppersThisRow > 0 && checkConstraints)
                    throw new SolverException("Can't have upper and lower " +
                            "bounds in the same row.");
                if (numLowersThisRow > 1) {
                    if (numLowersThisRow != cols && checkConstraints)
                        throw new SolverException("Allowed lower bounds " +
                        		"in a row: 0, 1, or number of columns.");
                ...
                else { /* There is only one bound in this row, a lower-bound.
                            It must be on the diagonal. */
                    if (i > cols || AI.doubleValue(i, i) > LOWER_BOUND_THRESHOLD
                            && checkConstraints)
                        throw new SolverException("The lower bound was " +
                        		"not on the lower's diagonal.");
                    /* We have not yet added this row to the growing list of
                     * rowsNeeded, so no need to subtract one from its size
                     * to get the index.         */
                    ensureIneqIndices(); // Inits indices to inequalities if null
            ...
            if (numUppersThisRow > 0 ) {
                if (numUppersThisRow > 1) {
                    if (numUppersThisRow != cols)
                        throw new SolverException("Allowed upper bounds " +
                            "in a row: 0, 1, or number of columns.");
                ...
                else { /* There is only one bound in this row, an upper bound.
                            It must be on the diagonal. */
                    if ((i < cols || AI.doubleValue(i, i - getSolutionSize()) < 
                            UPPER_BOUND_THRESHOLD) && checkConstraints)
                        throw new SolverException("The upper bound was " +
                                "not on the upper's diagonal.");
                    /* We have not yet added this row to the growing list of
                     * rowsNeeded, so no need to subtract one from its size
                     * to get the index.         */
                    ensureIneqIndices(); // Inits indices to inequalities if null
                    subfaultToUpperBIndices[i - getSolutionSize()] = 
                        rowsNeeded.size();
                }
            }
    ...
    /** Culls the rows of the inequality matrix that do not correspond
     * to any bound.  We need to eliminate unused rows so ojAlgo's quad. prog.
     * solver doesn't take 40 times longer to run.  
     * Copied and modified from purgeUnusedInequalityRows()  */
    private void purgeUnusedEqualityRows() {
        ...
        /* Start off with invalid indices for all possible 
            ...
            if (numNonZerosThisRow > 0) {
                if (numNonZerosThisRow > 1) {
                    if (numNonZerosThisRow != cols && checkConstraints)
                        throw new SolverException("Allowed non-zeros " +
                                "in a row: 0, 1, or number of columns.");
                ...
                else { /* There is only one non-zero in this row.
                            It must be on the diagonal. */
                    if (i > cols || AE.doubleValue(i, i) ==  0.0
                            && checkConstraints)
                        throw new SolverException("A subfault's equality " +
                                "constraint was not on the diagonal.");
                    /* We have not yet added this row to the growing list of
                     * rowsNeeded, so no need to subtract one from its size
                     * to get the index.         */
    ...
    public double getLowerBound(final int subfaultIndex) {
        if (BI == null) 
            return Double.NaN; 
        if (subfaultToLowerBIndices == null)
            if (AI.doubleValue(subfaultIndex,subfaultIndex) < 
                    LOWER_BOUND_THRESHOLD)
                return -BI.doubleValue(getLbRow(subfaultIndex), 0);
            else /* This is the right row, but no bound has been set. */
                return Double.NaN;
    ...
    public double getUpperBound(final int subfaultIndex) {
        ...
        if (subfaultToUpperBIndices == null)
            if (AI.doubleValue(subfaultIndex + getSolutionSize(), subfaultIndex) > 
    ...
    /** Gets the equation number for an upper bound, before the 
     * unused rows have been culled.  Does NOT check
     * whether AI is null, because the caller is probably going to have to
     * do that itself, anyway.  Created because we anticipate removing rows
     * for bounds not used, in the future, in which case this will help 
     * encapsulate those effects.  Will generally  crash (throw 
     * outOfBoundsException) if called after the solve() has been run; solve()
     * culls the unused equations from the inequality matrices.
     * 
     * @param subfaultIndex
   ...
   private int getLbRow(int subfaultIndex) {
    ...
    /**
     * @return NaN if there is no equality constraint on the total
     * moment.  If there is such a constraint the constraint divided by the product of shear modulus times
     * total area.
     */
