...
public class CrossValidator2 {
    ...
    public double calculate() {
        ...
        for (int gamIter = 0; gamIter < numGam; gamIter++) {
            ...
            for (int stationIter = 0; stationIter < stationCt; stationIter++) {
                ...
                DistributedSlipSolver solver = new DistributedSlipSolver(modelArray, shadedStationLoc, shadedRealDisp,
                        shadedCov, simModel, lowerbound, upperbound, nonNegReqList, mommentConList, conTypeList, true,
                        curGam);
    ...
    private CovarianceWeighter shadeCovar(CovarianceWeighter source, int index) {
        ...
        for (int rowIter = 0; rowIter < newLen; rowIter++) {
            int colOffset = 0;
            if (rowIter == index)
                rowOffset = AXIS_PER_STATION;
            for (int colIter = 0; colIter < newLen; colIter++) {
                if (colIter == index)
                    colOffset = AXIS_PER_STATION;
                newMatrix[rowIter][colIter] = fullMatrix[rowIter + rowOffset][colIter + colOffset];
            }
        }

        return new CovarianceWeighter(newMatrix); // TODO: use clone to create
                                                  // (now refstation info is
                                                  // lost)
    }
